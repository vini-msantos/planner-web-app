...
pub async fn fetch_bootstrap(pool: &SqlitePool) -> anyhow::Result<BootstrapData> {
    ...
    let columns = sqlx::query_as!(Column, "SELECT * FROM columns;")
        .fetch_all(pool)
        .await?
        .into_iter()
        .map(|c| (c.id.clone(), c))
    ...
    let tasks = sqlx::query_as_unchecked!(Task, "SELECT * FROM tasks;")
        .fetch_all(pool)
        .await?
        .into_iter()
        .map(|t| (t.id.clone(), t))
    ...
    let scheduled_activities =
        sqlx::query_as!(ScheduledActivity, "SELECT * FROM scheduled_activities;")
            .fetch_all(pool)
            .await?
            .into_iter()
    ...
    let routines = sqlx::query_as!(Routine, "SELECT * FROM routines;")
        .fetch_all(pool)
        .await?
        .into_iter()
        .map(|r| (r.id.clone(), r))
    ...
    let routine_activities = sqlx::query_as!(RoutineActivity, "SELECT * FROM routine_activities;")
        .fetch_all(pool)
        .await?
        .into_iter()
